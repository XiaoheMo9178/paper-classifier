...
项目分类目标是“研究主题”，而非传统学科。插件会优先归入稳定的一级主题池（如干预与试验研究、系统综述与证据综合、量表与测量工具、预测模型与诊断评估等），二级主题也使用固定受控分类表，避免每篇论文都创建零散目录。
...
- 使用聚焦主题池 + 固定二级分类表 + 同义归并，减少批量分类时的目录碎片
...
| 版本 | 安装包 | 插件 ID | 界面语言 |
|---|---|---|---|
| 中文版 | `paper-classifier-1.1.2.xpi` | `paper-classifier@example.com` | 中文 |
| 英文版 | `paper-classifier-1.1.2-en.xpi` | `paper-classifier-en@example.com` | 英文 |
...
4. 按 `主题根目录/聚焦一级主题/受控二级主题` 自动创建集合并归档
5. 弹出结果汇总（成功/失败总数、分了几类、每类多少篇）
...
├── paper-classifier-1.1.2.xpi        # 中文版安装包
└── paper-classifier-1.1.2-en.xpi     # 英文版安装包
```
